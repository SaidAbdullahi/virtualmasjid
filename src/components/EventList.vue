<template>
  <div class="event-list">
    ...
    <div class="my-content-container">
        ...
        <div class="container-fluid">
          <div class="results-header">
            <h2 class="results-number">
              <span class="event-total">{{ eventCount }}</span>
              <span class="d-none">matching</span> events
              <span v-if="selected_venue">
                at
                <a
                  class="header-venue"
                  href="#"
            ...
            <div class="sort">
              <a class="sort-button btn btn-outline-light" href="#" />
      ...
      <venue-modal :venue="selected_venue" />
    </div>
  </div>
</template>
...
const view = {
    ...
    function bubbles() {
      $.each($('.particletext.bubbles'), function() {
        ...
        for (let i = 0; i <= bubblecount; i += 1) {
          ...
          $(this).append(
            `<span class="particle" style="top:${top}%; left:${left}%;width:${size}px; height:${size}px;animation-delay: ${delay}s;"></span>`
          )
        }
      })
    }
  ...
  methods: {
    onQueryEnter() {
      ...
      searchEvents(this.query).then(response => {
    ...
    updateEvents() {
      if (this.selected_venue_id) {
        ...
        getEvents(this.selected_sort.sort, this.invert_sort).then(response => {
          this.eventCount = response.data.count
          this.events = response.data.results
          this.next = response.data.next
          // this.mungeEvents();
...
.list-inline > li:not(:last-of-type) {
  border-right: 0.5px solid #31302c;
}
...
.search-container input.search.filled + .btn-clearsearch,
.search-container.active .btn-clearsearch {
  display: block;
}
...
.search:focus,
.search:active,
select#taphouseSelect:focus,
select#taphouseSelect:active {
  outline: 0;
  background: #bbfae0;
}
...
.sort-options ul li a.selected {
  border-left: 0.5rem solid #02de7f;
}
...
.event.active .event-speaker,
.event.active .event-style {
  overflow: visible;
  white-space: normal;
}
/*
...
.mug-handle {
  ...
  transition: height 0.2s ease-out, transform 0.2s ease-out 0.2s,
    top 0.2s ease-out, border-radius 0.2s ease-out 0.2s,
    background 0.2s ease-out 0.2s, border 0.2s ease-out;
}
...
.mug-handle:after {
  ...
  left: 0;
  transition: height 0.2s ease-out, transform 0.2s ease-out 0.2s,
    top 0.2s ease-out, border-width 0.2s ease-out;
}
...
.mug-top:after {
  ...
  border-radius: 50%;
  z-index: 1;
  transition: border 0.2s ease-out, height 0.2s ease-out, top 0.2s ease-out,
...
.mug-top:after {
  ...
  border: 0;
  transition: border 0.2s ease-out, height 0.2s ease-out, top 0.2s ease-out,
    border-radius 0.2s ease-out, top 0.2s ease-out 0.2s,
...
.active .search-icon {
  margin-top: 0;
  margin-bottom: 0.5em;
  transform: scale(1) scaleZ(1) rotateX(-10deg);
...
.active .search-icon .mug-top:after {
  ...
  background: #fff;
  transition: height 0.2s ease-out 0.2s, top 0.2s ease-out,
    background 0.2s ease-out 0.4s, box-shadow 0.2s ease-out 0.4s,
    top 0.2s ease-out 0.6s;
}

.active .search-icon .mug-bottom {
  top: calc(6em - 2px);
  height: 1.5em;
  transition: height 0.2s ease-out 0.2s, top 0.2s ease-out 0.2s;
}
...
.active .search-icon .mug-handle {
  ...
  transition: height 0.2s ease-out 0.2s, border-width 0.2s ease-out 0.2s,
    transform 0.2s ease-out, top 0.2s ease-out 0.2s;
}
...
.active .search-icon .mug-handle:after {
  ...
  border-left: 0;
  height: 2.75em;
  transition: height 0.2s ease-out 0.2s, border-width 0s ease-out 0.2s,
...
.search-icon.dark .mug-top,
.search-icon.dark .mug-bottom,
.search-icon.dark .mug-sides,
.search-icon.dark .mug-handle {
  border-color: #31302c;
}
...
.taphouse-select-container:focus-within:after {
